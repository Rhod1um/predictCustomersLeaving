...
def predict(): # this function can have any name
  ...
  try:
    ...
    model = load_model('customerPrediction.h5') # the mymodel.h5 file was created in Colab, downloaded and uploaded using Filezilla
    ...
    if in1 == None or in2 == None: # check if any number is missing
      ...
      arr = [[float(in1), float(in2),float(in3), float(in4),float(in5), float(in6), float(in7), float(in8), float(in9), float(in10), float(in11), float(in12), float(in13)]] # cast string to decimal number, and make 2d numpy array.
